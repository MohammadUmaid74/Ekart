...
from app51.models import Product,Cart,Order
...
def makepayments(request):
    orders=Order.objects.filter(uid=request.user.id)
    s=100
    no =len(orders)  #no of products
    for x in orders:
        s=s+x.pid.pcost*x.qty 
        oid=x.order_id
    client = razorpay.Client(auth=("rzp_test_GyBrKORaS06a7C","HY0uDiZf2xnubIRW3KwCBOwj"))
    data = { "amount": max(s*100,100), "currency": "INR", "receipt": 'oid' }
